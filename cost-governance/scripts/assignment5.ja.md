...
id: 3yf8ktrlyu1r
type: challenge
title: アクセス制御と SKU 管理
notes:
- type: text
  contents: |
    # コストガバナンス

    Datadog で SKU を有効にする方法は複数あります。これは意図的な設計で、Dev、Ops、Sec の各チームがプロダクトを有効化し、必要な可視性を得られるようにするためです。

    この柔軟性は価値がありますが、想定外のコストも生み得ます。ROI の最大化と、誤った機能の有効化の防止に焦点を当てます。

    このセッションでは、誤って有効にした機能の無効化と今後の有効化防止、および中核的なセキュリティのベストプラクティスを扱います。
tabs:
- id: a2cupr6nweqb
  title: Terminal
  type: terminal
