...
- type: text
  contents: |
    # Out-of-the-box検知ルールを探索し、攻撃を実行する

    このラボでは、AWSログの脅威を分析するために使用されるOOTB Cloud SIEM検知ルールを探索します。その後、AWS環境に対して攻撃をステージングします。

    最初のアクティビティでは、次のことを行います：
      - AWSの脅威を検知するために使用されるOOTB検知ルールを探索する
      - ラボAWSアカウントに対して攻撃をエミュレートする
